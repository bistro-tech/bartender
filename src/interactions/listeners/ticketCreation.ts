...
export const CREATE_TICKET: Listener = {
	...
	execute: async (interaction) => {
		...
		if (!kind) return LOGGER.interaction.fatal(interaction, `CREATE_TICKET without a kind??`);
		...
		LOGGER.interaction.debug(
			interaction,
			`user ${formatUser(interaction.member.user)} requested the creation of ${ticketName}.`,
		);
